    ...
    while True:
        ...
        while last_ts <= now - 1:
            for symbol in symbols:
                ...
                # Решаем, хит ли свеча (и при необходимости пишем в ORDERS_2)
                is_hit = False
                if c['volume'] != 0.0:
                    try:
                        if check_candle(c):
                ...
                # Логи: полный — только для хитов; для прочих — короткий раз в LOG_EVERY секунд
                if is_hit:
                    print(
                    ...
                    # короткий лог — не чаще, чем раз в LOG_EVERY секунд по КАЖДОМУ символу
                    if (last_ts - _last_log_ts[symbol]) >= _LOG_THROTTLE:
                        print(f"[{when}] {symbol} V:{c['volume']} X_V20:{c['X_V_20']} P_OC:{c['P_OC']}%")
                        _last_log_ts[symbol] = last_ts
