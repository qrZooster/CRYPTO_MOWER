...
import bb_db as db
from bb_trade import trader   # ← ДОБАВЬ ЭТУ СТРОКУ
...
def insert_candle(symbol: str, c: dict, table_name: str = 'ZZ$CANDELS_S1'):
    ...
    row = {
        ...
        'FLD$DIRECTION': c['D'],
        'FLD$TCOD': symbol,
        'FLD$OPERATION': c.get('OPERATION', 'NONE'),
        'FLD$LEVERAGE': int(c.get('LEVERAGE', 0)),
        'FLD$SUM': round(float(c.get('SUM', 0.0)), 2),
    }
    return db.qr_add(table_name, row)

def open_trade(symbol: str, c: dict):
    """Открыть ордер по свече c, если заданы OPERATION и SUM > 0.
    Работает через bb_trade.trader(); по умолчанию DRY-RUN."""
    try:
        if c.get('OPERATION') != 'NONE' and float(c.get('SUM', 0) or 0) > 0:
            tr = trader()  # DRY-RUN; для реальной торговли init_trader(dry_run=False)
            res = tr.open_order_from_c(symbol, c)
            print(f"[TRADE] {symbol} op={c.get('OPERATION')} lev={c.get('LEVERAGE')} sum={c.get('SUM')} -> {res}")
            return res
...
def check_candle(c: dict) -> bool:
    ...
    # Если прошли все фильтры — решаем направление и плечо
    if (x_v_20 > 1000 and p_oc >= 1.0):
        c['OPERATION'] = 'BUY' if c.get('D') == 'UP' else 'SELL'
        c['LEVERAGE'] = 10
        c['SUM'] = 6.5
    return True
