...
class TGrid(TCompositeControl):
    ...
    def td(self, index: int | None = None) -> "TGrid_Td | None":
        """
        Доступ к ячейке в ПОСЛЕДНЕЙ строке грида.

        grid.td()      → создаёт новую TGrid_Td в последней строке.
        grid.td(i)     → возвращает ячейку i последней строки (или None, если её нет).

        Строка, с которой работаем, всегда grid.tr(-1).
        Если строк ещё нет — сначала создаётся первая строка.
        """
        rows = getattr(self, "Rows", None)

        # если строк ещё нет — создаём первую
        if not rows:
            row = self.tr()       # создаст TGrid_Tr(self) + первую ячейку
        else:
            # всегда работаем с последней строкой
            row = rows[-1]

        # дальше делегируем логику индексирования строке
        return row.td(index)
    # ..................................................................................................................
    # 🎨 Рендеринг грида
    # ..................................................................................................................
...
class TGrid_Td(TFlex_Td):
    ...
    def _offset_style_dict(self) -> dict[str, str]:
        """
        Для ячейки грида логические top/left/right/bottom трактуем как
        ВНУТРЕННИЕ отступы (padding-*), а не margin-*.
        """
        style: dict[str, str] = {}
