...
# --- Параметры ---
WRITE_TO_DB = os.getenv("SCAN7_WRITE", "0") == "1"  # включить запись в БД
COOL = 5
MAX_WINDOW = 100
HISTORY_SAFETY = 10
...
# ТИК-ДЕТЕКТОР: параметры (env-override)
BURST_W_SHORT_MS = int(os.getenv("BURST_W_SHORT_MS", "200"))   # окно короткое, мс (0.3s)
BURST_W_LONG_MS  = int(os.getenv("BURST_W_LONG_MS",  "1500"))  # окно длинное, мс (1.5s)
BURST_X_RATE_MIN = float(os.getenv("BURST_X_RATE_MIN", "4.0"))  # rate_short / max(rate_long,eps)
BURST_X_VR_MIN   = float(os.getenv("BURST_X_VR_MIN",   "3.0"))  # volrate_short / max(volrate_long,eps)
BURST_MICRO_POC  = float(os.getenv("BURST_MICRO_POC",  "0.6"))  # % прирост цены за короткое окно
BURST_TTL_SEC    = float(os.getenv("BURST_TTL_SEC",    "2.0"))  # TTL факта TICK_BURST

# Запись BURST в БД (управление)
BURST_WRITE      = os.getenv("BURST_WRITE", "1") == "1"       # писать ли тик-бурсты в БД
BURST_MIN_SEP_MS = int(os.getenv("BURST_MIN_SEP_MS", "1000"))  # минимум между вставками по одному символу
_last_burst_ms = defaultdict(lambda: 0)

# РАННИЙ ВХОД (экстренный) — запись в ORDERS_2
EARLY_WRITE      = os.getenv("EARLY_WRITE", "1") == "1"       # писать ли ранние входы в ZZ$ORDERS_2
EARLY_TRADE      = os.getenv("EARLY_TRADE", "0") == "1"       # реально ли торговать (по умолчанию — нет)
EARLY_SUM_USDT   = float(os.getenv("EARLY_SUM_USDT", "6.5"))    # сумма на вход
EARLY_LEVERAGE   = int(os.getenv("EARLY_LEVERAGE", "10"))       # плечо
...
def check_delisting_once():
    ...
    try:
        ...
        print(f"[SCAN7][delisting] fetch error: {e}")
    # прогреем факты (последние 60 дней)
    #if db._db is None:
    #    db.init_db()
    try:
        rows = qr(
            "SELECT `FLD$SYMBOL`, `FLD$DATE_TIME` "
            "FROM `ZZ$DELISTING` "
        ...
        print(f"[SCAN7][delisting] cache load error: {e}")
    ...
    def on_trade(self, symbol: str, ts_ms: int, price: float, volume: float, db=None):
        ...
        try:
            ...
            if ok:
                ...
                print(f"[BURST] {symbol} xr={feats['x_rate']} xv={feats['x_vr']} micro={feats['micro_p']}% tps={feats['rate_s']}")
                insert_tick_burst(symbol, ts_ms, price, float(volume), feats)
        except Exception as e:
            print(f"[SCAN7][burst] error: {e}")
            ...
            for symbol in symbols:
                ...
                if WRITE_TO_DB:
                    try:
                        ...
                        print(f"[SCAN7] DB insert error: {e}")
...
def insert_tick_burst(symbol: str, ts_ms: int, price: float, qty: float, feats: dict):
    ...
    try:
        ...
        rw = {
            ...
            FLD_TCOD: tcod(symbol, ts_ms // 1000, 'BURST', BYBIT),
            FLD_VERSION: VERSION,
...
def insert_early_order(symbol: str, ts_ms: int, price: float, qty: float, feats: dict):
    ...
    try:
        ...
        rw = {
            ...
            FLD_TCOD: tcod(symbol, dt_sec, 'EARLY', BYBIT),
            FLD_VERSION: VERSION,
        ...
        print(f"[EARLY] {symbol} op={operation} lev={EARLY_LEVERAGE} sum={EARLY_SUM_USDT} poc={poc}%")
    except Exception as e:
        print(f"[EARLY][DB] {symbol} err: {e}")
...
def insert_candle(symbol: str, c: dict, table_name: str = 'ZZ$CANDELS_S1', tf: str = '1SEC'):
    ...
    rw = {
        ...
        FLD_TCOD: tcod(symbol, c['start'], tf, BYBIT),
        FLD_VERSION: VERSION,
